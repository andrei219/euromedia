...
class FPDF(object):
    ...
    def add_page(self, orientation='', print_lines_header=True):
        "Start a new page"
        if(self.state==0):
            self.open()
        ...
        self.color_flag=cf
        #Page header
        self.header(print_lines_header=print_lines_header)
        #Restore line width
    ...
    @check_page
    def cell(self, w, h=0, txt='', border=0, ln=0, align='', fill=0, link='', caller=None, print_lines_header=False):
        "Output a cell"
        txt = self.normalize_text(txt)
        k=self.k
        if(self.y+h>self.page_break_trigger and not self.in_footer and self.accept_page_break()):
            ...
            self.add_page(self.cur_orientation, print_lines_header = print_lines_header)

            self.set_xy(*INNER_LINE_START)
            if caller:
                caller.y_start = INNER_LINE_START[1]
            self.x=x
            if(ws>0):
                self.ws=ws
